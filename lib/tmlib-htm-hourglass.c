...
tm_startup(size_t num_threads) {
    ...
    for(size_t threadId = 0; threadId < num_threads; ++threadId) {
        ...
        printf("[TMLIB] in_hourglass (%p)\n", &(gp_tm_threads[threadId].in_hourglass));
    }

    tm_init_flock();
    printf("[TMLIB] Initialized htm-hourglass (%u)\n", TMLIB_ABORT_THRESHOLD);
...
tm_wait(uint32_t arg, tm_thread_t* p_tm_thread, pthread_spinlock_t* p_spinlock) {
    ...
    } else if(arg == TMWAIT_ACTIVE_HOURGLASS) {
        volatile uint32_t* p_hourglass = &(gp_tm_state->hourglass);
        uint32_t hourglass = *p_hourglass;
        __sync_synchronize();
        while(hourglass == 1) {
            tm_random_delay(p_tm_thread->myid);
            hourglass = *p_hourglass;
            __sync_synchronize();
        }
...
static int
tm_acquire_flag(tm_thread_t* p_tm_thread) {
    volatile uint32_t* p_hourglass = &(gp_tm_state->hourglass);
...
tm_release_flag(tm_thread_t* p_tm_thread) {
    ...
    assert(p_tm_thread->in_hourglass == 1);
    assert(*p_hourglass);
...
tm_begin(long threadId, pthread_spinlock_t* p_fallback_lock) {
    ...
    if(p_tm_thread->depth == 1) {
        ...
        do {
            ...
            uint32_t hourglass = *p_hourglass;
            __sync_synchronize();
            if((p_tm_thread->in_hourglass == 0) && (hourglass == 1)) {
                tm_wait(TMWAIT_ACTIVE_HOURGLASS, p_tm_thread, p_fallback_lock);
            }
